...
void Tmy_G::filter_on_off(bool stat)
{
  _filter_delta = stat;
}
...
int Tmy_G::cari_idx_a_2(int idx_b, Treturn_update_rho rho, vector<T_alpha_container> alpha, vector<T_grad_container> grad, Tmy_kernel *kernel, Tmy_alpha *my_alpha)
{
  auto cek_filter = [](callback_param var_b, callback_param var_a, vector<T_alpha_container> alpha, vector<T_grad_container> grad, Tmy_kernel *kernel, Tmy_alpha *my_alpha) -> bool
  {
    ...
    if (is_pass)
    {
      is_pass = !alpha[0].is_lb(var_b.idx) and !alpha[0].is_ub(var_b.idx);
    }
    ...
    if (is_pass)
    {
      ...
      if (is_pass)
      {
        ...
        if (hsl_eta[4] < 0.0)
        {
          ...
          tmp_v1 = my_alpha->is_pass(var_b.idx, var_a.idx, delta_v1, alpha[1]);
          tmp_v1.reset();

          Tmy_double delta_v2 = hsl_eta[0] * (grad[2][var_a.idx] - grad[2][var_b.idx]);
          tmp_v2 = my_alpha->is_pass(var_b.idx, var_a.idx, delta_v2, alpha[2]);
          tmp_v2.reset();

          Tmy_double delta = hsl_eta[0] * (var_a.grad - var_b.grad);
          //  cout << "delta " << delta << endl;

          tmp = my_alpha->is_pass(var_b.idx, var_a.idx, delta, alpha[0]);
        }
        else
        {
          Tmy_double c1 = hsl_eta[4] / 2.0;

          Tmy_double hsl_sum = (grad[1][var_a.idx] - grad[1][var_b.idx]);
          Tmy_double c2 = hsl_sum - hsl_eta[4];
          tmp_v1 = my_alpha->is_pass(var_b.idx, var_a.idx, c1, c2, alpha[1]);
          tmp_v1.reset();

          hsl_sum = (grad[2][var_a.idx] - grad[2][var_b.idx]);
          c2 = hsl_sum - hsl_eta[4];
          tmp_v2 = my_alpha->is_pass(var_b.idx, var_a.idx, c1, c2, alpha[2]);
          tmp_v2.reset();

          hsl_sum = (var_a.grad - var_b.grad);
          c2 = hsl_sum - hsl_eta[4];

          tmp = my_alpha->is_pass(var_b.idx, var_a.idx, c1, c2, alpha[0]);
        }
        is_pass = tmp.is_pass;
...
int Tmy_G::max(Tmy_double rho1, Tmy_double rho2, vector<T_alpha_container> alpha, vector<T_grad_container> grad, callback_type f)
{
  ...
  for (int i = 0; i < _jml_data; ++i)
  {
    Tmy_double G = grad[0][i];
    Tmy_double dec_F = grad[0].dec(i, rho1, rho2);
    Tmy_double obj_F = grad[0].obj(i, rho1, rho2);
...
int Tmy_G::max(int idx_b, Tmy_double rho1, Tmy_double rho2, vector<T_alpha_container> alpha, vector<T_grad_container> grad, Tmy_kernel *kernel, callback_type f)
{
  ...
  for (int i = 0; i < _jml_data; ++i)
  {
    Tmy_double Ga_v1 = grad[1][i];
    Tmy_double Ga_v2 = grad[2][i];
    Tmy_double Ga = grad[0][i];
    Tmy_double dec_Fa = grad[0].dec(i, rho1, rho2);
    Tmy_double obj_Fa = grad[0].obj(i, rho1, rho2);
    ...
    if (is_pass)
    {
      ...
      if ((abs_diff_F >= gmax))
      {
        ...
        if (_filter_delta)
        {
          ...
          tmp_delta.push_back(delta_v1);
          tmp_delta.push_back(delta_v2);
          is_pass = delta_filter(idx_b, i, alpha, tmp_delta);
        }
        if (is_pass)
        {
          cout << " idx_a " << var_a.idx << " delta " << delta << " delta v1 " << delta_v1 << " delta v2 " << delta_v2 << endl;
...
int Tmy_G::cari(int idx_b, Tmy_double rho1, Tmy_double rho2, vector<T_alpha_container> alpha, vector<T_grad_container> grad, Tmy_kernel *kernel, Tmy_alpha *my_alpha, callback_type1 f)
{
  ...
  int startIndex = 1 + (rand() % _jml_data);
...
bool Tmy_G::delta_filter(int idx_b, int idx_a, vector<T_alpha_container> alpha, vector<Tmy_double> delta)
{
  ...
  // is_pass_1 = (abs(alpha[1][idx_b]) <= abs(delta[1])) or (abs(alpha[1][idx_a]) <= abs(delta[1]));
  // is_pass_2 = (abs(alpha[2][idx_b]) <= abs(delta[2])) or (abs(alpha[2][idx_a]) <= abs(delta[2]));

  is_pass_0 = (abs(alpha[0][idx_b]) <= abs(delta[0])) or (abs(alpha[0][idx_a]) <= abs(delta[0]));

  is_pass = is_pass_0; // or is_pass_1 or is_pass_2;
