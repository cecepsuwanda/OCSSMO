...
int T_grad_container::max(Tmy_double rho1, Tmy_double rho2, T_alpha_container alpha, callback_type f)
{
	...
	for (int i = 0; i < _idx.size(); ++i)
	{
		...
		bool is_pass = f(-1, tmp_idx[i], F, 0.0, alpha);
		if ((abs_F >= gmax) and is_pass)
		{
			gmax = abs_F;
			idx_max = tmp_idx[i];
			mv_idx(tmp_idx[i], 0);
...
int T_grad_container::max(int idx_b, Tmy_double rho1, Tmy_double rho2, T_alpha_container alpha, Tmy_kernel *kernel, callback_type f)
{
	Tmy_double gmax = -HUGE_VAL;
	int idx_max = -1;
	...
	{		
		...
		bool is_pass = f(idx_b, tmp_idx[i], Fa, Fb, alpha);
		if (is_pass)
		{
			if (diff_F >= gmax)
			{
...
int T_grad_container::max(int idx_b, Tmy_double rho1, Tmy_double rho2, T_alpha_container alpha, Tmy_kernel *kernel, Tmy_alpha *my_alpha, callback_type1 f)
{
	Tmy_double gmax = -HUGE_VAL;
	int idx_max = -1;
	...
	for (int i = 0; i < _idx.size(); ++i)
	{
		Tmy_double Fa = obj(i, rho1, rho2);
		Tmy_double diff_F = abs(Fb - Fa);
		bool is_pass = f(idx_b, tmp_idx[i], Fa, Fb, alpha, kernel, my_alpha);
		if (is_pass)
		{
			if (diff_F >= gmax)
			{
...
int T_grad_container::cari(int idx_b, Tmy_double rho1, Tmy_double rho2, T_alpha_container alpha, Tmy_kernel *kernel, Tmy_alpha *my_alpha, callback_type1 f)
{
	int idx_a = -1;
	...
	for (int i = 0; i < _idx.size(); ++i)
	{
		...
		bool is_pass = f(idx_b, _idx[i], Fa, Fb, alpha, kernel, my_alpha);
		if (is_pass)
		{
			idx_a = _idx[i];
			break;
