...
int Tmy_svm::take_step(int idx_b, int idx_a)
{
   ...
   else
   {
      ...
      // cout << "delta v1 " << delta << endl;
      Treturn_is_pass tmp_v1 = _my_alpha->is_pass(idx_b, idx_a, delta, _alpha_v1);

      hsl_sum = _my_G.sum_alpha_diff_Q(_alpha_v2, hsl_diff);
      delta = hsl_eta[0] * hsl_sum;
      // cout << "delta v2 " << delta << endl;
      Treturn_is_pass tmp_v2 = _my_alpha->is_pass(idx_b, idx_a, delta, _alpha_v2);
      ...
      // if(abs(tmp_v1.new_alpha_i)<1e-3) tmp_v1.new_alpha_i = 0.0;
      // if(abs(tmp_v1.new_alpha_j)<1e-3) tmp_v1.new_alpha_j = 0.0;
      // if(abs(tmp_v2.new_alpha_i)<1e-3) tmp_v2.new_alpha_i = 0.0;
      // if(abs(tmp_v2.new_alpha_j)<1e-3) tmp_v2.new_alpha_j = 0.0;
   ...
   while ((iter < max_iter) and (is_alpha_changed > 0))
   {
      ...
      else
      {
         for (int idx_b = 0; idx_b < jml_data; idx_b++)
         {
            ...
            is_pass = !_alpha_v1.is_lb(idx_b) and !_alpha_v1.is_ub(idx_b);
            if (!is_pass)
            {
               is_pass = !_alpha_v2.is_lb(idx_b) and !_alpha_v2.is_ub(idx_b);
            }
            if (is_pass)
            {
               // cout << " rho v1 " << _rho.rho_v1 << " rho v2 " << _rho.rho_v2;
